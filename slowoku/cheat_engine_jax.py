...
class CheatEngine:
    ...
    def __init__(self, wordlist):
        ...
        self.val_mask = jnp.ones(len(wordlist), dtype=bool)
    ...
    def apply_green(self, guess, result):
        ...
        """
        green_mask = \
            jnp.all((guess == self.wordlist[self.val_mask])[:, result == 2],
                    axis=1)
        self.val_mask = self.val_mask.at[self.val_mask].set(green_mask)

    def apply_yellow(self, guess, result, g_occurrs, y_occurrs):
        # delete words where y_letter is on the same position:
        yellow_repeated_mask = jnp.all((self.wordlist[self.val_mask] != guess)
                                      [:, result == 1], axis=1)
        self.val_mask = self.val_mask.at[self.val_mask].set(yellow_repeated_mask)
        for y_letter, y_count in y_occurrs.items():
            ...
            yellow_ok_mask = jnp.sum(
                (self.wordlist[self.val_mask] == y_letter),
                axis=1) >= threshold
            self.val_mask = self.val_mask.at[self.val_mask].set(yellow_ok_mask)

    def apply_black(self, guess, result, g_occurrs, y_occurrs, b_occurrs):
        # delete words where b_letter is on the same position:
        black_repeated_mask = jnp.all((self.wordlist[self.val_mask] != guess)
                                     [:, result == 0], axis=1)
        self.val_mask = self.val_mask.at[self.val_mask].set(black_repeated_mask)
        for b_letter, _ in b_occurrs.items():
            ...
            black_ok_mask = jnp.sum((self.wordlist[self.val_mask] == b_letter),
                                    axis=1) <= threshold
            # self.wordlist = self.wordlist[black_ok_mask]
            self.val_mask = self.val_mask.at[self.val_mask].set(black_ok_mask)
